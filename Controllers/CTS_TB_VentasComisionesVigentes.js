// CTS_TB_VentasComisionesVigentes.js
/*
 ...
 *
 * Descripción:
 * Controladores CRUD y utilitarios para la tabla ventas_comisiones_vigentes.
 *
 * Tema: Controladores - Comisiones Vigentes (por período/mes)
 * Capa: Backend (Node.js + Sequelize)
 */

import db from '../DataBase/db.js';
import { Op } from 'sequelize';
import { VentasComisionesVigentesModel } from '../Models/MD_TB_ventas_comisiones_vigentes.js';
 ...
 */
export const OBRS_ComisionesVigentes_CTS = async (req, res) => {
  try {
    ...
    const data = await VentasComisionesVigentesModel.findAll({
      where,
      order: [
        ['periodo_inicio', 'DESC'],
        ['codigo', 'ASC']
...
/**
 * Obtener una comisión vigente por ID.
 */
export const OBR_ComisionVigente_CTS = async (req, res) => {
  try {
    const id = Number(req.params.id);
    const item = await VentasComisionesVigentesModel.findByPk(id);
    if (!item) {
 ...
 *   - activo (boolean)
 */
export const CR_ComisionVigente_CTS = async (req, res) => {
  ...
  try {
    ...
    const existente = await VentasComisionesVigentesModel.findOne({
    ...
    const nuevo = await VentasComisionesVigentesModel.create(
      {
...
/**
 * Actualizar comisión vigente por ID.
 ...
 */
export const UR_ComisionVigente_CTS = async (req, res) => {
  const t = await db.transaction();
  try {
    const id = Number(req.params.id);
    const item = await VentasComisionesVigentesModel.findByPk(id, {
    ...
    await VentasComisionesVigentesModel.update(cambios, {
      where: { id },
      transaction: t
    });

    const refreshed = await VentasComisionesVigentesModel.findByPk(id, {
      transaction: t
    });
    await t.commit();
...
/**
 * Eliminar comisión vigente (hard delete por defecto).
 * Si preferís soft delete, reemplazar por update { activo: 0 }.
 */
export const ER_ComisionVigente_CTS = async (req, res) => {
  try {
    const id = Number(req.params.id);
    const n = await VentasComisionesVigentesModel.destroy({ where: { id } });
    if (!n) {
      return res.status(404).json({ mensajeError: 'Registro no encontrado' });
    }
 ...
 */
export const DUP_Comisiones_Mes_CTS = async (req, res) => {
  const t = await db.transaction();
  try {
    const {
      origen_mes,
    ...
    const origen = await VentasComisionesVigentesModel.findAll({
    ...
    for (const o of origen) {
      const ya = await VentasComisionesVigentesModel.findOne({
        where: { periodo_inicio: pDestino, codigo: o.codigo },
        transaction: t
      });
      if (ya) continue;

      await VentasComisionesVigentesModel.create(
        {
          codigo: o.codigo,
          titulo: o.titulo,
...
/**
 * Desactivar todas las comisiones de un período (bulk).
 * Query/body:
 *   - mes, anio  (o periodo_inicio directo)
 */
export const DESACTIVAR_Comisiones_Mes_CTS = async (req, res) => {
  const t = await db.transaction();
  try {
    ...
    const [n] = await VentasComisionesVigentesModel.update(
      { activo: false },
...
/**
 * Obtener comisión por código para un período.
 * Query:
 *   - codigo (requerido)
 *   - mes, anio (si no vienen, usa mes actual)
 */
export const OBR_ComisionPorCodigo_CTS = async (req, res) => {
  try {
    ...
    const item = await VentasComisionesVigentesModel.findOne({
      where: {
        periodo_inicio: periodo,
        codigo: String(codigo).trim(),
        activo: true
